...
public class BaiduMapTool {
	...
	public static void initLocationPosition(Context context) {
		// 定位初始化, 20秒扫描一次
		LocationClient mLocationClient = initLocationClient(context, 60000);
		...
		mLocationClient.registerLocationListener(new BDLocationListener() {
			...
			public void onReceiveLocation(BDLocation location) {
				...
				// System.out.println(address);
				 System.out.println("=1=地址： " + address + "， 纬度：" + latitude + "， 经度：" + longitude);
			}
...
//		textView.setTextColor(activity.getResources().getColor(R.color.white));		// 设置字体颜色
	 ...
	 * @param address			图片弹出窗上显示的 地址信息
	 */
	public static  void showButton(Context context, BaiduMap baiduMap, LatLng latLng) {
		// ========增加 弹出窗覆盖物=========
		// 创建InfoWindow展示的view  
		Button button = new Button(context);
//		button.setBackgroundResource(R.drawable.icon_marker1); // 设置按钮背景
		
		// 定义用于显示该InfoWindow的坐标点  
		// 创建InfoWindow , 传入 view， 地理坐标， y 轴偏移量 
		InfoWindow mInfoWindow = null;				// 显示窗口，用于显示 具体的地址 
//		 mInfoWindow = new InfoWindow(BitmapDescriptorFactory.fromView(button), latLng, -2, null);
		 mInfoWindow = new InfoWindow(button, latLng, -1);
		// 显示InfoWindow  
		baiduMap.showInfoWindow(mInfoWindow);
	}
	...
	public static String getShiFromAddress() {
		...
		int start = (index1>0) ? index1 : 0;
		String addr = address.substring(start+1, index2);
