...
public class PossibleKnowAdapter extends BaseAdapter implements
	...
	public PossibleKnowAdapter(Context context) {
	...
	public PossibleKnowAdapter addMember(List<Member> m) {
		mMembers.addAll(m);
		return this;
	}
	...
	public View getView(int position, View view, ViewGroup parent) {
		...
		final Holder holder;
		if (view == null) {
			view = inflater.inflate(R.layout.possible_konw_person, null);
			holder = new Holder();
		...
		if (member.getIsColleague().trim().equals("0")) {
			holder.distance.setText(member.getFriendCount() + "个共同好友");
		} else {
			holder.distance.setText("同事");
		}

		holder.nick.setText(member.getRealName());
